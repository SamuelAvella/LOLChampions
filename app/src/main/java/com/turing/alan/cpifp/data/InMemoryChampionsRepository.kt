...
class InMemoryChampionsRepository private constructor(): ChampionsRepository {
    ...
    override fun getChampions(): List<Champion> {
        return _champions.toList() // Devuelve una copia de la lista de campeones
    }

    override fun readOne(id: Int): Champion {
        return _champions.single { it.id == id } // Encuentra el campeón con el ID dado
    }

    override fun readAll(): List<Champion> {
        return _champions.toList() // Devuelve una copia de la lista de campeones
    }
}
